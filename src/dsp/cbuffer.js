export default function CBuffer() {
  ...
  if (arguments.length > 1 || typeof arguments[0] !== 'number') {
    this.data = new Float64Array(arguments.length)
    this.end = (this.length = arguments.length) - 1
    this.push.apply(this, arguments)
  } else {
    this.data = new Float64Array(arguments[0])
...
CBuffer.prototype = {
  ...
  reverse: function() {
    ...
    for (; i < ~~(this.size / 2); i++) {
      ...
      this.data[(this.start + i) % this.length] = this.data[
        (this.start + (this.size - i - 1)) % this.length
      ]
  ...
  unshift: function() {
    ...
    for (i = 0; i < arguments.length; i++) {
      this.data[(this.length + this.start - (i % this.length) - 1) % this.length] =
        arguments[i]
    }
